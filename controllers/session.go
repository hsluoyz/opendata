...
func (c *ApiController) GetSessions() {
	...
	if limit == "" || page == "" {
		...
		paginator := pagination.SetPaginator(c.Ctx, lim, count)
		sessions, err := object.GetPaginationSessions(owner, paginator.Offset(), lim, field, value, sortField, sortOrder)
		if err != nil {
			c.ResponseError(err.Error())
			return
		}
...
// @Tag Session API
// @Description Get session for one user.
...
// @Title UpdateSession
// @Tag Session API
// @Description Update session for one user.
// @Param   id     query    string  true        "The id(organization/user) of session"
// @Success 200 {array} string The Response object
...
// @Title AddSession
// @Tag Session API
// @Description Add session for one user. If there are other existing sessions, join the session into the list.
// @Param   body    body   object.Session  true        "The details of the session"
// @Success 200 {array} string The Response object
...
// @Title DeleteSession
// @Tag Session API
// @Description Delete session for one user.
// @Param   body    body   object.Session  true        "The details of the session"
// @Success 200 {array} string The Response object
...
// @Tag Session API
// @Description Check if there are other different sessions for one user.
// @Param   id     query    string  true        "The id(organization/user) of session"
