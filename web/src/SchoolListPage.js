...
class SchoolListPage extends BaseListPage {
  ...
  renderTable(schools) {
        ...
        render: (text, record) => (
          <div>
            <Button
              ...
              type="primary"
              onClick={() => this.props.history.push(`/schools/${record.owner}/${record.name}`)}
            >
              编辑
            </Button>
            <Popconfirm
    ...
    return (
      <div>
        <Helmet><title>{Setting.getPageTitle("学校列表")}</title></Helmet>
        <Table
          ...
          title={() => (
            <div>
              学校列表&nbsp;&nbsp;&nbsp;&nbsp;
              <Button type="primary" size="small" onClick={() => this.addSchool()}>添加</Button>
            </div>
          )}
        />
      </div>
