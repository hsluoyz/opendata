...
class StudentListPage extends BaseListPage {
  ...
  renderTable(students) {
        ...
        render: (text, record) => (
          <div>
            <Button
              ...
              type="primary"
              onClick={() => this.props.history.push(`/students/${record.owner}/${record.name}`)}
            >
              编辑
            </Button>
            <Popconfirm
    ...
    return (
      <div>
        <Helmet><title>{Setting.getPageTitle("学生列表")}</title></Helmet>
        <Table
          ...
          title={() => (
            <div>
              学生列表&nbsp;&nbsp;&nbsp;&nbsp;
              <Button type="primary" size="small" onClick={() => this.addStudent()}>添加</Button>
            </div>
          )}
        />
      </div>
