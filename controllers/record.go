...
func (c *ApiController) GetRecords() {
	...
	if limit == "" || page == "" {
		...
		records, err := object.GetPaginationRecords(owner, paginator.Offset(), lim, field, value, sortField, sortOrder)
		if err != nil {
			c.ResponseError(err.Error())
			return
		}
...
func (c *ApiController) UpdateRecord() {
	...
	affected, err := object.UpdateRecord(id, &record)
...
func (c *ApiController) AddRecord() {
	...
	affected, err := object.AddRecord(&record)
	if err != nil {
		c.ResponseError(err.Error())
		return
	}
...
func (c *ApiController) DeleteRecord() {
	...
	affected, err := object.DeleteRecord(&record)
	c.Data["json"] = wrapActionResponse(affected, err)
	c.ServeJSON()
}
