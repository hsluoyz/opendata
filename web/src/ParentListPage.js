...
class ParentListPage extends BaseListPage {
  ...
  renderTable(parents) {
        ...
        render: (text, record) => (
          <div>
            <Button
              ...
              type="primary"
              onClick={() => this.props.history.push(`/parents/${record.owner}/${record.name}`)}
            >
              编辑
            </Button>
            <Popconfirm
    ...
    return (
      <div>
        <Helmet><title>{Setting.getPageTitle("家长列表")}</title></Helmet>
        <Table
          ...
          title={() => (
            <div>
              家长列表&nbsp;&nbsp;&nbsp;&nbsp;
              <Button type="primary" size="small" onClick={() => this.addParent()}>添加</Button>
            </div>
          )}
        />
      </div>
